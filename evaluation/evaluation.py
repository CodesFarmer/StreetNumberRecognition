...
# proposal_pnet = hd.create_pnet(sess, 'model/model_pnet.ckpt')
...
for img_name in content:
    ...
    if numbox > 0:
        ...
        area_iou = dg.IoU(bbx, total_boxes[0, :])
        with open('%s/time_consuming/%s_quality.txt' % (root_dir, whichop), 'a') as file:
        ...
        # for k in range(0, numbox):
        #     cv2.rectangle(img_show, (int(total_boxes[k, 0]), int(total_boxes[k, 1])),
        #                   (int(total_boxes[k, 2]), int(total_boxes[k, 3])), (0, 0, 255), 1)
        #
        ...
        with open('%s/time_consuming/%s_failure.txt' % (root_dir, whichop), 'a') as file:
            file.write('%s\n' % img_name)